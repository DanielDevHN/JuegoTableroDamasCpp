...
ElementoA::ElementoA(int x, int y)
{
...
vector<int *> ElementoA::obtenerMovimientosValidos(int **mapTablero)
{

	// Se declara el segundo arreglo del tablero
	int **tablero = mapTablero;
	...
	for (int j = 0; j < listaSinTerminar.size(); j++)
	{
		...
		cuadroActual[1] = listaSinTerminar[j][1] + 1; // y hacia abajo
		...
		for (int i = 0; i < 2; i++)
		{
			...
			if (estaLaCoordenadaEnTablero(cuadroActual[0], cuadroActual[1]) == true && j == 0 && tablero[cuadroActual[0]][cuadroActual[1]] == 0)
			{
			...
			else if (estaLaCoordenadaEnTablero(cuadroActual[0] + direction, cuadroActual[1] + 1) == true && tablero[cuadroActual[0]][cuadroActual[1]] * equipo < 0 && tablero[cuadroActual[0] + direction][cuadroActual[1] + 1] == 0)
			{
				...
				movimientoListado[contadorDeMovimientos][1] = cuadroActual[1] + 1; // abajo
